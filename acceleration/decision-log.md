...
| D-014 | Which JSON field names the structured logger emits for the timestamp, the message body, and the run-scoped correlation ID | (a) Canonical aggregator names — `timestamp` / `message` / `correlation_id` (the literal field names common in Datadog, Loki, OpenSearch, and Splunk source-type defaults); (b) Compact pipeline-internal names — `ts` / `msg` / `run_id` (terser, fewer keystrokes per log line, common in newer JSON-log conventions including bunyan-style and structlog defaults); (c) Both — emit canonical names with compact aliases for backwards compatibility | (b) — `ts` / `level` / `logger` / `msg` / `run_id` / `extra`. AAP §0.7.1 Rule 1 does not mandate specific field names; the logger's documented contract in [`observability/logger.py`](./observability/logger.py) (lines 16-22, 222-260), [`observability/README.md`](./observability/README.md) §3.1, and [`observability/dashboard.html`](./observability/dashboard.html) (which consumes `run_id` directly) is internally consistent on this choice. | The compact names match the convention used inside the analysis pipeline's batch context (single process, JSON-line emission to `stderr`, no log-shipper in this configuration). They are also the names that the companion `dashboard.html` parses from `data/run_manifest.json`. Emitting both sets of names would double the byte cost per line and risk silent divergence when one is updated but not the other. The QA report at Checkpoint 1 (Issue M-1) explicitly recognises this as a "literal vs documented" divergence rather than a functional defect, and recommends preserving the logger surface and aligning the checkpoint instructions instead. | Downstream log aggregators (Datadog, Loki, OpenSearch, Splunk) that have default source-type parsers keyed on `timestamp` / `message` / `correlation_id` would require a small parser remap or a `processors` rule. Mitigated by (i) the consistent and documented field-name contract, (ii) the JSON-line format which any modern aggregator can ingest with a one-line parser configuration, and (iii) the run-scoped correlation field which can be re-aliased at ingestion time. | `observability/logger.py` (`JsonFormatter.format`); `observability/README.md` §3.1 (documented field-name contract); `observability/dashboard.html` (run-log-tail consumer) |
