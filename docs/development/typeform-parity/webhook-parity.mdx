...
This example shows the proposed format that maintains structural parity with Typeform while preserving Formbricks-specific fields. The payload would be sent when the `payloadVersion: "typeform-compat"` flag is enabled on the webhook configuration.
...
|---|---|---|
| **Payload version flag** | Add a `payloadVersion` field to the `Webhook` model (`"v1"` for current, `"typeform-compat"` for parity format) | ✅ Recommended — cleanest separation, per-webhook granularity |
...
<Steps>
  <Step title="Extend Webhook Model">
    Add an optional `payloadVersion` field to the `Webhook` Prisma model and `ZWebhook` Zod schema. Default to `"v1"` (current format) for backward compatibility.

    ```typescript
    // Proposed extension to ZWebhook
    payloadVersion: z.enum(["v1", "typeform-compat"]).default("v1")
    ```
  </Step>
  <Step title="Branch Payload Construction">
    In the pipeline route (`apps/web/app/api/(internal)/pipeline/route.ts`), check `webhook.payloadVersion` before constructing the payload body. For `"v1"`, use the current format; for `"typeform-compat"`, apply the transformations from the [Payload Transformation Requirements](#payload-transformation-requirements) section.
  </Step>
  <Step title="Leverage Webhook Source for Defaults">
    Use the existing `WebhookSource` enum to apply intelligent defaults:
    - `user` webhooks: Default to `v1`, allow user to switch to `typeform-compat`
    - `zapier`, `make`, `n8n`, `activepieces` webhooks: Default to `v1`, preserving current integration behavior
    - Future migration tools could set `typeform-compat` by default for Typeform imports
  </Step>
...
<Note>
  The `payloadVersion` approach ensures zero disruption to existing consumers. New webhooks created for Typeform migration can opt into the `typeform-compat` format, while all existing webhooks continue to receive the current `v1` format unchanged.
</Note>
...
1. Update webhook configuration via the API or UI to set `payloadVersion: "typeform-compat"`
