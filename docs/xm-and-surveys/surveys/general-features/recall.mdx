...
Not all question types can serve as recall sources. The following question types support recall:
...
<Note>
  The following question types do **not** support recall: File Upload, Statement/CTA, Consent, Picture Selection, Schedule a Meeting (Cal.com), and Matrix.
</Note>

You can insert recalled data into the **title** or **description** field of any question in your survey.
...
Type **`@`** in the question or description field where you want to insert a recall. This triggers a dropdown menu listing all preceding questions, hidden fields, and variables. Select the item you want to recall data from.
...
To ensure the survey remains coherent when a response is missing (or the question is optional), you should set a fallback value. The fallback text is displayed to the respondent whenever the recalled data is unavailable.
