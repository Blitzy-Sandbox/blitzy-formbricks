...
* Copy the Client ID, Client Secret, and Redirect URL and set them as environment variables in your Formbricks instance (e.g., in the docker-compose file):

  * `GOOGLE_SHEETS_CLIENT_ID` — OAuth Client ID

  * `GOOGLE_SHEETS_CLIENT_SECRET` — OAuth Client Secret

  * `GOOGLE_SHEETS_REDIRECT_URL` — Set to `https://<your-public-facing-url>/api/google-sheet/callback`
