...
To enable email functionality, configure the following environment variables in your `.env` file or `docker-compose.yml`:

| Variable | Type | Default | Description |
| --- | --- | --- | --- |
| `MAIL_FROM` | `string` | — | Email address used as the sender for all outgoing emails (e.g., `noreply@yourdomain.com`) |
| `MAIL_FROM_NAME` | `string` | `Formbricks` | Display name shown alongside the sender email address |
| `SMTP_HOST` | `string` | — | Hostname of your SMTP server (e.g., `smtp.sendgrid.net`) |
| `SMTP_PORT` | `number` | `1025` | Port for the SMTP server. Common values: `587` (STARTTLS), `465` (TLS), `25` (unencrypted) |
| `SMTP_USER` | `string` | — | Username for SMTP authentication |
| `SMTP_PASSWORD` | `string` | — | Password for SMTP authentication |
| `SMTP_SECURE_ENABLED` | `0 \| 1` | `0` | Set to `1` to enable TLS for SMTP connections (typically used with port `465`) |
| `SMTP_AUTHENTICATED` | `0 \| 1` | `1` | Set to `0` to skip SMTP authentication (disables `SMTP_USER` and `SMTP_PASSWORD` requirement) |
| `SMTP_REJECT_UNAUTHORIZED_TLS` | `0 \| 1` | `1` | Set to `0` to accept connections without requiring authorization from the list of supplied CAs |

<Note>
  The first six variables (`MAIL_FROM` through `SMTP_PASSWORD`) are required to enable email functionality. The remaining three are optional and control advanced TLS and authentication behavior.
</Note>

Here is an example `.env` configuration block:
...
For a comprehensive list of all environment variables, see the [Environment Variables](/self-hosting/configuration/environment-variables) reference.
...
<Warning>
  For Gmail, you must use an [App Password](https://support.google.com/accounts/answer/185833) instead of your regular account password. This is required if you have 2-Factor Authentication enabled, which Google strongly recommends for all accounts.
</Warning>
