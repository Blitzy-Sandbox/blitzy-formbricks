...
<Step title="Check for survey trigger">
  ...
  <Step title="Check for user or visitor qualification">
    If the user or visitor already filled out a survey in the past couple of days (see [Survey Cooldown](/xm-and-surveys/surveys/website-app-surveys/recontact#project-wide-survey-cooldown)), the survey will not be shown to prevent survey fatigue.
...
<Tip>
  ...
  research is relevant to the current user context which leads to **significantly higher response and
  completion rates** as well as lower survey fatigue.
</Tip>
...
For more granular control, you can implement actions directly in your code. Code actions use the `formbricks.track()` method to programmatically send action events to the Formbricks SDK.

The `track` method has the following TypeScript signature:

```typescript
track(code: string, properties?: TTrackProperties): Promise<void>
```

- **`code`**: A unique string identifier for the action (e.g., `"Button Clicked"`, `"Plan Upgraded"`).
- **`properties`** *(optional, deprecated)*: An object containing `hiddenFields` to pass additional data. Hidden fields support will be removed in a future version.

The method returns a `Promise`, so it can be awaited to ensure the action has been processed before continuing.

{/* Source: packages/js-core/src/index.ts (track function), apps/web/modules/survey/editor/lib/action-class.ts */}
...
![Add a code action to open source in app survey](/images/xm-and-surveys/surveys/website-app-surveys/actions/code-action.webp "Add a code action to open source in app survey")
  ...
  <Step title="Add action tracking to your code">
    ...
    You can also use `async`/`await` to ensure the action is tracked before continuing:
