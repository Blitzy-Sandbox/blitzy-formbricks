...
<Note>Formbricks also offers an API v2 (currently in Beta) with enhanced capabilities. See the [API v2 Reference](/api-v2-reference/introduction) for details.</Note>

## Data Flow

### Create Survey With Formbricks

Create a survey in Formbricks (UI) or programmatically via the Management API. Read more about the API endpoint [here](/api-reference/management-api--survey/create-survey).  
...
Get the survey schema using the Formbricks API. Read more about the API endpoint [here](/api-reference/management-api--survey/get-survey-by-id).
...
More about the Endpoint [here](/api-reference/client-api--environment/get-environment-state).
...
Post responses directly from the browser to Formbricks Client API. Read more about it [here](/api-reference/client-api--response/create-response).  
...
Configure webhooks in Formbricks to push response data to your system. Read more about Webhooks [here](/xm-and-surveys/core-features/integrations/webhooks#webhooks).

Formbricks webhooks follow the [Standard Webhooks](https://github.com/standard-webhooks/standard-webhooks) specification, using HMAC-SHA256 signing for payload verification.
...
Fetch responses periodically from your backend, read more about the Endpoint [here](/api-reference/management-api--response/get-survey-responses).
