...
1. **Individual URLs**: Each contact receives a unique survey link tied to their contact record. Under the hood, the link contains an encrypted JWT token embedding both the contact ID and the survey ID, signed with HMAC-SHA256
2. **Automatic attribution**: Responses are automatically linked to the specific contact who clicked the link. When the contact opens the URL, Formbricks decrypts the token to resolve the contact record and associates the submitted response with that contact in the database — no additional identification step is required
3. **Single-use by default**: Each link can only be used once to prevent duplicate responses. If a contact has already responded to the survey, the API rejects further link generation for that contact
4. **Expiry control**: Set expiration dates to control survey access windows. Expiry is enforced at the JWT token level, so expired links return a clear error message

Once responses are collected, you can view them in the **Responses** tab of your survey. Each response displays the associated contact's attributes (name, email, custom ID), enabling targeted follow-up analysis and segment-based reporting directly in the Formbricks dashboard.

<!-- Source: apps/web/modules/ee/contacts/lib/contact-survey-link.ts, apps/web/modules/api/v2/management/surveys/[surveyId]/contact-links/ -->
...
You can also generate Personal Links programmatically via the API v2. Two endpoints are available:

- **Individual contact**: [Get personalized survey link for a contact](/api-v2-reference/management-api--surveys--contact-links/get-personalized-survey-link-for-a-contact) — generates a link for a single contact by `contactId` with an optional `expirationDays` query parameter
- **Segment-based**: [Get survey links for contacts in a segment](/api-v2-reference/management-api--surveys--contact-links/get-survey-links-for-contacts-in-a-segment) — generates links for all contacts within a given segment

Both endpoints require a valid API key with appropriate environment permissions. Refer to the [API v2 Reference](/api-v2-reference/introduction) for authentication details.

<!-- Source: apps/web/modules/api/v2/management/surveys/[surveyId]/contact-links/ -->
