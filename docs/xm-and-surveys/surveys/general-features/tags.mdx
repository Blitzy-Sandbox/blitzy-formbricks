...
2. Click the **Merge** button next to the tag
3. Search for and select the destination tag from the popup
...
<Steps>
  ...
  <Step title="Select a tag">
    In the filter dropdown, select a tag from the **Tags** category.
  </Step>
  
  <Step title="Choose filter type">
    Choose whether to filter by:
    - **Applied**: Show only responses that have this tag
    - **Not applied**: Show only responses that do not have this tag
  </Step>
  
  <Step title="Apply and view results">
    Click **Apply Filters** to update the response list with your tag filter.
  </Step>
</Steps>
