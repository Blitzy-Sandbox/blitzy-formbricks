...
<Steps>
  ...
  <Step title="Create Zod schema">
    ...
    - `scaleRange: z.union([z.literal(5), z.literal(7), z.literal(10)])` — configurable scale endpoints
    ...
    - `visualStyle: z.enum(["number", "smiley", "star"])` — visual rendering style for scale options
    - `isColorCodingEnabled: z.boolean().optional().default(false)` — optional color coding from red to green
  </Step>
...
<Steps>
  ...
  <Step title="Create Zod schema">
    ...
    - `currency: z.enum(["usd", "eur", "gbp"])` — ISO 4217 currency code
    - `amount: z.number().int().positive().min(1)` — payment amount in the smallest currency unit
    - `buttonLabel: ZI18nString.optional()` — custom label for the payment button
    - `stripeIntegration: z.object({ publicKey: z.string(), priceId: z.string() })` — Stripe integration configuration
  </Step>
...
<Steps>
  ...
  <Step title="Update download UI">
    Add a JSON option to the export dropdown in `apps/web/app/(app)/environments/[environmentId]/surveys/[surveyId]/components/CustomFilter.tsx`.
  </Step>
