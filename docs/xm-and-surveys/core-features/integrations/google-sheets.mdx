...
When a survey response value exceeds 49,995 characters, it is automatically truncated to fit within the cell. This truncation happens transparently during the data sync — no action is required on your part.
...
2. **Google Spreadsheet API**: To write to the spreadsheet you select (that's it, nothing else, we're opensource, see this method in our codebase [here](https://github.com/formbricks/formbricks/blob/main/apps/web/lib/googleSheet/service.ts#L72))
