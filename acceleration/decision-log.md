...
| D-014 | Which JSON field names the structured logger emits for the timestamp, the message body, and the run-scoped correlation ID, and which output stream to use | (a) Canonical aggregator names only — `timestamp` / `name` / `message` / `correlation_id` routed to `stdout` (the literal field names common in Datadog, Loki, OpenSearch, and Splunk source-type defaults); (b) Compact pipeline-internal names only — `ts` / `logger` / `msg` / `run_id` routed to `stderr` (terser, fewer keystrokes per log line, common in bunyan-style and structlog JSON-log conventions); (c) Both — emit canonical names with compact aliases for backwards compatibility, routed to `stdout` | (c) — emit BOTH `timestamp` and `ts`, BOTH `name` and `logger`, BOTH `message` and `msg`, BOTH `correlation_id` and `run_id` on every record, with `level` shared across both alias sets. Route the StreamHandler to `sys.stdout`. The two alias sets agree byte-for-byte on every record. AAP §0.7.1 Rule 1 does not mandate specific field names; this choice maximises compatibility with both downstream log aggregators (whose default source-type parsers key on the canonical names) and the existing internal consumers ([`observability/dashboard.html`](./observability/dashboard.html), [`observability/README.md`](./observability/README.md) §3.1) that were originally written against the compact names. | The initial pipeline-internal-only choice produced a "literal vs documented" divergence flagged in the Checkpoint 1 QA review (Issue M-1). Emitting both sets eliminates the divergence at the small cost of a few extra bytes per line: the aggregator-friendly names allow Datadog / Loki / OpenSearch / Splunk to parse the records without a custom rule, while the compact aliases preserve every existing internal consumer unchanged. Stdout was chosen for the StreamHandler so that any log shipper attached to the process picks the lines up via the conventional stdout channel; scripts that need to emit non-log machine-readable output for a caller MUST write to an explicit file path rather than relying on stdout being log-free. | Slightly larger per-line byte cost than either single-alias-set design (≈80 extra bytes per record for the four duplicated values). Mitigated by (i) computing each canonical value exactly once and binding it under both names, so there is no risk of silent divergence between the two aliases, (ii) the `JsonFormatter.format` method documenting the alias relationship inline, and (iii) the existing JSON-line format which keeps records human-readable for any aggregator. Stdout-routing could in principle conflict with a script that also writes machine-readable output to stdout; mitigated by the orchestrator's run-manifest capture and by the explicit guidance that such scripts use file paths rather than stdout. | `observability/logger.py` (`JsonFormatter.format` emits both alias sets; `_configure_root_logger` routes to `sys.stdout`); `observability/README.md` §3.1 (documented field-name contract for both alias sets); `observability/dashboard.html` (run-log-tail consumer, already accepts either alias set via `entry.ts \|\| entry.timestamp` style fallbacks) |
| D-015 | How the reveal.js theme stylesheet at [`templates/deck/theme.css`](./templates/deck/theme.css) loads the three required Google Fonts (Inter, Space Grotesk, Fira Code) while satisfying the QA checkpoint's self-containment constraint | (a) Use CSS `@import url(... fonts.googleapis.com ...)` at the top of `theme.css` so the stylesheet self-loads its fonts when used standalone; (b) Use a `<link rel="stylesheet" href="fonts.googleapis.com/...">` tag in the parent HTML's `<head>` block and have `theme.css` declare only `font-family` stacks; (c) Inline the font files (base64-encoded `@font-face`) directly into `theme.css` for true offline self-containment | (b) — `theme.css` contains zero `@import` directives. The parent HTML (`acceleration/executive-presentation.html`, and any future `render_deck.py`-generated deck) is the sole site that loads Google Fonts via `<link rel="stylesheet">` tags in its `<head>` block. `executive-presentation.html` already implements this pattern at its `<head>` (with `<link rel="preconnect">` plus `<link href="...fonts.googleapis.com/css2?family=Inter...Space+Grotesk...Fira+Code...">`). `theme.css` documents the required families and weights in its top file-header comment so any future inliner knows what `<link>` tags the parent must emit. | The QA Checkpoint 2 instructions explicitly state: "Verify NO `@import` directives... Any `@import` is a CRITICAL finding (breaks render_deck.py self-containment guarantee)." Option (a) violates this constraint regardless of how the rationalization is phrased. Option (c) would require checking ~300 KB of base64-encoded woff2 binaries into the repository and would defeat the CDN delivery `display=swap` semantics that prevent flash-of-invisible-text. Option (b) is the only choice that is BOTH self-contained (per the QA checkpoint) AND Rule 5-compliant (per AAP §0.7.1, which mandates Google Fonts loading but does not specify the CSS `@import` mechanism). It is also already implemented in `executive-presentation.html`. | Browsers that load `theme.css` in isolation (no parent HTML) render with fallback typefaces (`system-ui`, `-apple-system`, `'Segoe UI'`, etc. from the `--font-body`, `--font-display`, `--font-mono` stacks) rather than the brand fonts. Mitigated by (i) the explicit fallback chain on every typography custom property, which keeps text legible without the brand fonts; (ii) the file-header comment that documents the required `<link>` tags so any future inliner emits them; (iii) the existence of `executive-presentation.html` as the canonical consumer, which already implements the correct pattern. | [`templates/deck/theme.css`](./templates/deck/theme.css) (zero `@import` directives, file-header comment documents required `<link>` tags); [`executive-presentation.html`](./executive-presentation.html) (parent HTML emits the three Google Fonts `<link>` tags in `<head>`) |
| D-016 | Which patch version of Mermaid to load via CDN, given that the AAP §0.6.1 literal `11.4.0` pin and the Rule 5 verbatim text both name `11.4.0`, but `11.4.0` has been retroactively classified as vulnerable to known CVEs | (a) Keep the literal `11.4.0` pin and accept the known CVEs because the analysis pipeline does not render attacker-controlled Mermaid input; (b) Upgrade to `11.15.0` (the smallest version that backports the security fixes per the Mermaid 11.x line) and document the deviation from the AAP's literal pin; (c) Upgrade to the latest available Mermaid (no fixed floor) and accept future-version drift; (d) Migrate to the long-term-support `10.9.6` backport line | (b) — Upgrade every Mermaid CDN reference and every narrative version mention across `acceleration/` from `11.4.0` to `11.15.0`. The bump targets — all now updated — are: the two renderer source files of truth (`scripts/render_deck.py` `CDN_MERMAID` constant; `scripts/verify_report.py` `PINNED_MERMAID_VERSION` constant), the generated `executive-presentation.html` (CDN URL plus four narrative references — regenerated by `render_deck.py` from the upgraded constant), `observability/dashboard.html` (CDN URL plus five narrative references), `observability/README.md`, `README.md`, `templates/deck/theme.css` (header comment), `templates/deck/slide_01_title.html.tmpl`, `templates/deck/slide_03_kpis.html.tmpl`, `templates/deck/slide_04_inflection.html.tmpl`, `templates/deck/slide_08_architecture.html.tmpl`, `templates/deck/slide_12_engineers.html.tmpl`, `templates/mermaid/acceleration_curve.mmd.tmpl`, `templates/mermaid/pipeline_architecture.mmd.tmpl`, and ND-008 in this decision log. The `render_deck.py` constant and the `verify_report.py` pin are the source of truth that determines the live CDN URL embedded in the generated deck and the version the verifier enforces; the narrative templates and one-time-rendered HTML files now match those constants. The xychart-beta syntax used by the Acceleration Curve template is stable across the 11.x line, so the upgrade is a drop-in replacement. The Mermaid 11.x runtime API (`mermaid.initialize`, `mermaid.run`) is unchanged from `11.4.0` to `11.15.0`. | The Mermaid maintainers published CVE-2026-41148 (HTML/CSS injection via `classDef`), CVE-2026-41149 (HTML injection via state-diagram `classDef`), and CVE-2026-41150 (Gantt-chart infinite-loop DoS) after the AAP was authored. All three CVEs affect Mermaid 11.0.0-alpha.1 through 11.14.0 and are fixed in 11.15.0 and 10.9.6. The CVE-2026-41150 fix is directly relevant because the Inflection Date slide (`slide_04_inflection.html.tmpl`) renders a Gantt chart from runtime-substituted dates. Although the substituted dates originate from in-repo `inflection.json` rather than untrusted user input, retaining a CVE-affected pin when the upstream non-vulnerable line is a drop-in replacement is a defect by the principle of least exposure. Per AAP §0.7.1 Rule 3 ("Any deviation from a literal or obvious interpretation of the requirements MUST have an explicit entry in the decision log"), this entry is the required documentation of the deviation from the AAP §0.6.1 literal `11.4.0` pin. The Checkpoint 3 FINAL review flagged a residual gap where the renderer constants and verifier pin still pointed at `11.4.0` even though the narrative documentation referenced `11.15.0`; that residual gap is now closed by updating those two constants and regenerating the deck. | Future Mermaid 11.x releases may introduce breaking changes; the `11.15.0` pin protects against drift while preserving the security fix. The deck's runtime regression surface is unchanged because the API contract used (`mermaid.initialize`, `mermaid.run`) has identical signatures across the 11.x line. The xychart-beta block in `templates/mermaid/acceleration_curve.mmd.tmpl` was re-verified against the 11.15.0 release notes; no syntax changes affect the lines that block emits. If a future security advisory affects 11.15.0, this entry's pattern is the precedent for the next override. The renderer-side pin acts as a regression guard — any future Checkpoint that regenerates the deck against a downgraded constant would fail `verify_report.py` immediately. | `scripts/render_deck.py` (`CDN_MERMAID` constant — the source of truth for the embedded CDN URL); `scripts/verify_report.py` (`PINNED_MERMAID_VERSION` — the verifier pin that enforces consistency); `executive-presentation.html` (generated artifact; regenerated to match the upgraded renderer constant); `observability/dashboard.html`; `observability/README.md`; `README.md`; `templates/deck/theme.css`; `templates/deck/slide_01_title.html.tmpl`; `templates/deck/slide_03_kpis.html.tmpl`; `templates/deck/slide_04_inflection.html.tmpl`; `templates/deck/slide_08_architecture.html.tmpl`; `templates/deck/slide_12_engineers.html.tmpl`; `templates/mermaid/acceleration_curve.mmd.tmpl`; `templates/mermaid/pipeline_architecture.mmd.tmpl` |
| D-019 | Whether `templates/deck/slide_09_flow_metrics.html.tmpl` emits the 3-column-per-metric grid (`*_DISPLAY`, `*_CONF`, `*_CONF_LABEL` — 21 tokens across the 7 Flow Framework metrics) that matches the QA Checkpoint 3 Phase 2 literal token-count expectation, or a 4-column grid that adds an explicit per-metric Caveat column (`*_DISPLAY`, `*_CONF`, `*_CONF_LABEL`, `*_CAVEAT` — 28 tokens across the same 7 metrics) | (a) Emit the 3-column 21-token grid and rely on the Risk Assessment section of `acceleration-report.md` to surface caveats for Low / Insufficient-signal metrics; (b) Emit the 4-column 28-token grid with a Caveat column populated from `metrics.json:"metrics.<name>.caveat"` (and rendered blank for High-confidence metrics); (c) Emit the 3-column grid in the deck and a 4-column grid in the report, so the deck stays terser at the cost of an internal-consistency exception | (b) — Emit the 4-column 28-token grid. Each row exposes `{{Mn_DISPLAY}}`, `{{Mn_CONF}}`, `{{Mn_CONF_LABEL}}`, and `{{Mn_CAVEAT}}` for n in 1..7. The renderer substitutes `{{Mn_CAVEAT}}` from `metrics.json:"metrics.<name>.caveat"`; when a metric is High-confidence with no caveat the renderer substitutes the empty string and the cell renders blank. The slide's leading comment block (lines 5-14, 31-79) documents all 28 tokens explicitly so no orphan tokens exist. | AAP §0.7.2.2 Rule 3 (Confidence Transparency) mandates that "every derived metric MUST carry a confidence tag" AND that "Low-confidence metrics MUST NOT appear without an explicit caveat." Slide 09 is the executive-view surface for Flow Framework metrics 1-7 — it displays the multiplier values directly to the leadership audience. Option (a) would omit the per-row caveat from the slide, so a Low-confidence or Insufficient-signal metric would appear in the deck stripped of its mandated Rule 3 caveat. Option (c) would create a Rule 4 (Internal Consistency) violation between the deck and the report, both of which are rendered from `metrics.json`. Option (b) is the only structure that satisfies Rule 3 on the surface that displays the metric values themselves and that preserves Rule 4 internal consistency. The 7-extra-token cost (28 vs 21) is a structural enhancement of an existing metric row, not the addition of a metric beyond the 12 specified — the AAP §0.7.2.1 boundary "MUST NOT add metrics beyond the 12 specified" remains satisfied because the metric count is unchanged. | A reader who applies the QA Checkpoint 3 Phase 2 literal 21-token expectation may classify the 28-token implementation as a defect rather than as a Rule 3-mandated enhancement. Mitigated by (i) the in-template comment header at lines 5-14 enumerating all four token suffixes per metric explicitly; (ii) this decision-log entry, which is the AAP §0.7.1 Rule 3 (Explainability) artifact that promotes the deviation rationale out of the template comment; (iii) the Caveat-column-empty rendering, which keeps the slide visually clean for the High-confidence rows so the extra column has zero visual cost when no caveats apply. | `templates/deck/slide_09_flow_metrics.html.tmpl` (header documents 28 tokens; body emits the 4-column table); `scripts/render_deck.py` (will substitute `{{M[1-7]_CAVEAT}}` from `metrics.json:"metrics.<name>.caveat"` per the Rendering notes in the template) |
...
| ND-008 | The reveal.js deck's visual identity is fixed by Rule 5 and is not redesigned | AAP §0.7.1 Rule 5 and §0.3.3 | Brand palette (`#5B39F3` primary, `#2D1C77` dark, `#94FAD5` teal, `#1A105F` navy), typography (Inter, Space Grotesk, Fira Code), and CDN-pinned library version family (reveal.js 5.1.0, Mermaid 11.x, Lucide 0.460.0) are dictated by the rule. The deck implements them rather than choosing them. The specific Mermaid 11.x patch version was raised from the AAP's literal 11.4.0 to 11.15.0 in D-016 below; the brand palette, typography, and other library versions remain untouched. |
...
| ND-011 | Verification screenshots produced during checkpoint review work reside under `blitzy/screenshots/` rather than under `acceleration/` | AAP §0.5.1 (Exhaustively In Scope — confines the analysis pipeline to `acceleration/`) and the Blitzy Screenshot Protocol (mandates `/tmp/blitzy/.../blitzy/screenshots/` as the persistence path) | The `blitzy/screenshots/` directory pre-dates this milestone and is the operational path required by the agent's Screenshot Protocol for verification evidence — it is not part of the analysis pipeline's output and is not consumed by any script under `acceleration/`. The files are visual-fidelity verification artifacts captured per Rule 5 / UD4 during prior checkpoint review work (theme.css and slide template renders at 1920×1080, mobile 375, tablet 768). They are retained as audit evidence rather than removed. The directory is excluded from the analysis pipeline's read scope (no extractor traverses it) and does not affect any reported metric. |
| ND-012 | The decision log itself receives an entry during a template-only checkpoint when the underlying decision affects a template or its consumer | AAP §0.7.1 Rule 3 (Explainability — "Every non-trivial implementation decision MUST be documented with rationale. Any deviation from a literal or obvious interpretation of the requirements MUST have an explicit entry in the decision log. Unexplained deviations are treated as defects.") | A non-trivial decision uncovered while working on a template (for example, D-015's font-loading mechanism for `theme.css`) MUST be recorded in this log per Rule 3, even when the milestone scope nominally covers only templates. Failing to record the decision would be a Rule 3 violation. The decision log is listed in AAP §0.5.1 as in-scope for the project as a whole; per-checkpoint scoping cannot override the global Rule 3 documentation requirement. |
