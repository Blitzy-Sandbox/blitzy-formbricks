...
<Note>
  The `currency` enum uses lowercase ISO 4217 codes (`"usd"`, `"eur"`, `"gbp"`) matching the OpenAPI specification. The `amount` field is an integer representing the smallest currency unit (e.g., cents), so `1000` represents $10.00 USD. The `stripeIntegration` object requires both `publicKey` (Stripe publishable key) and `priceId` (Stripe Price ID) as mandatory fields.
</Note>
...
| Rule | Description |
|---|---|
| Amount ≥ 1 | Amount must be a positive integer (smallest currency unit) |
| Valid currency | Currency must be one of the supported codes: `usd`, `eur`, `gbp` |
| Stripe keys configured | `stripeIntegration.publicKey` and `stripeIntegration.priceId` are required |
...
| Labels | Lower and upper (optional) | Lower and upper (optional) |
| Color coding | Optional (`isColorCodingEnabled`) | Optional (`isColorCodingEnabled`) |
| Visual style | Numbers only | Numbers, smileys, or stars (`visualStyle`) |
...
<Note>
  The `scaleRange` property accepts integer values `5`, `7`, or `10` representing the number of points on the scale (starting from 1). The `visualStyle` property controls the visual presentation: `"number"` for numeric labels, `"smiley"` for emoji faces, or `"star"` for star icons. These property names and enum values are consistent with both the v1 and v2 OpenAPI specifications.
</Note>
...
classDiagram
    ...
    class ZSurveyPaymentElement {
        ...
        +amount: integer
        +stripeIntegration: PublicKey + PriceId
    }
