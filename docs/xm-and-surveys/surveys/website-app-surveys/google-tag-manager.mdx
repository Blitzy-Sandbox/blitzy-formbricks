...
- Wait up to 1 minute for the server-side cache to refresh
...
Source: SDK API methods (`setup()`, `setUserId()`, `setAttributes()`, `track()`) are defined in `packages/js-core/src/index.ts`. The `TConfigInput` interface (`{ environmentId, appUrl }`) is defined in `packages/js-core/src/types/config.ts`.
